...
import { DefaultDocxParserBackend } from '../backends/docxBackend';

export class DocxDocumentParser extends BaseDocumentParser {
  readonly parserId = 'DOCX_PARSER_ENTERPRISE';
  readonly parserVersion = '1.0.0-build017b.1';
  protected readonly backend = new DefaultDocxParserBackend();
  ...
  public async validate(file: File | Uint8Array): Promise<boolean> {
    try {
      ...
      // DOCX is a ZIP container; check PK magic bytes
      return (
        header[0] === 0x50 &&
        header[1] === 0x4b &&
        header[2] === 0x03 &&
